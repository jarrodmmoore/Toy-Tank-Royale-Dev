scoreboard players set #damage_source value 2

#execute unless entity @s[scores={targetID=1..}] run tp @s ^ ^ ^.25
execute unless entity @s[scores={targetID=1..}] run tp @s ^ ^ ^.32
execute if entity @s[scores={targetID=1..}] run function ttr:pellet/homing_pellet_target
...
execute if score @s lifetime matches 1.. run scoreboard players add @s lifetime 1
