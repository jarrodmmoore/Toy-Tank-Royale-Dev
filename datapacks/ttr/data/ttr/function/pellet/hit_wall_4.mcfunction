...
execute if block ~ ~1 ~ #ttr:breakable run tag @s add do_not_bounce
...
execute unless entity @s[scores={bounceCooldown=1..}] unless entity @s[tag=do_not_bounce] if score #dx value > #dz value run function ttr:pellet/reflect/try_x
execute unless entity @s[scores={bounceCooldown=1..}] unless entity @s[tag=do_not_bounce] unless score #dx value > #dz value run function ttr:pellet/reflect/try_z

execute unless entity @s[scores={bounceCooldown=1..}] unless entity @s[tag=do_not_bounce] run scoreboard players set @s bounceCooldown 1

tag @s[tag=bouncy] remove bouncy
tag @s[tag=do_not_bounce] remove do_not_bounce
