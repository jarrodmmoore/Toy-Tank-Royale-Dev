scoreboard players set #damage_source value 1
#i don't like having to do this, but we're doing extra tag checks just to get more accurate hitfeed icons
execute if entity @s[tag=basic_pellet_burst] run scoreboard players set #damage_source value 10
execute if entity @s[tag=basic_pellet_triple] run scoreboard players set #damage_source value 11
...
execute at @s unless block ~ ~1 ~ #ttr:pellet_okay run function ttr:pellet/hit_wall_1
