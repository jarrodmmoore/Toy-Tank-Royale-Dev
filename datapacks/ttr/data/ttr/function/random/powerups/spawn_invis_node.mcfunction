...
#spawn the item
execute as @e[limit=1,tag=item_spawnpoint,type=marker] at @s positioned ~ ~-1.5 ~ run summon area_effect_cloud ~ ~ ~ {Tags:["ai_random_shoot","set_r_id"],custom_particle:{type:"block",block_state:{id:"air"}},WaitTime:0,Duration:100}
