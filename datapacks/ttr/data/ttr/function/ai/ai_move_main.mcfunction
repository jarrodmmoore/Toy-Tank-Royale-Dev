...
#execute as @e[tag=my_char,nbt={OnGround:1b},scores={moveState=1..}] at @s unless block ~ ~ ~ water run scoreboard players set @s moveState 1
#execute as @e[tag=my_char,nbt={OnGround:0b},scores={moveState=1..}] unless entity @s[scores={moveState=3}] at @s unless block ~ ~ ~ water run scoreboard players set @s moveState 2
#execute as @e[tag=my_char,scores={moveState=1..}] at @s if block ~ ~ ~ water run scoreboard players set @s moveState 4

#input limits and decay
#scoreboard players remove @s[scores={limitDelay=1..}] limitDelay 1
#execute unless entity @s[scores={mvmt_dx=0}] if entity @s[scores={limitDelay=..0}] run function ttr:movement/limit_dx
#execute unless entity @s[scores={mvmt_dy2=0}] if entity @s[scores={limitDelay=..0}] run function ttr:movement/limit_dy2
#execute unless entity @s[scores={mvmt_dz=0}] if entity @s[scores={limitDelay=..0}] run function ttr:movement/limit_dz
...
execute unless score @s aiLookID matches 1.. run function ttr:ai/aim/main_no_target
#execute if entity @s[tag=validRotation,scores={deathTime=..0}] at @e[tag=my_char,limit=1,sort=nearest] run function ttr:movement/aim/main
#execute unless entity @s[tag=validRotation] at @e[tag=my_char,limit=1,sort=nearest] run tp @e[limit=1,sort=nearest,tag=my_model_2] ~ ~-1.2 ~
scoreboard players remove @s aiDelayAim 1

#get y motion
#execute as @e[tag=my_char,limit=1] store result score @e[limit=1,tag=self101] motion_y run data get entity @s Motion[1] 1000
