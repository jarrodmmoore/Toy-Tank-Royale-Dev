...
#are we infected?
execute if entity @s[tag=redTeam] run scoreboard players set #decision value 1
execute if entity @s[tag=!redTeam] run scoreboard players set #decision value 2

#1 -- we're infected! go some place where there's hopefully STUFF (like another player to shoot at)
scoreboard players set #decisionB value 1
execute if score #decision value matches 1 unless entity @e[tag=player_char,tag=!self_shmoves,tag=!player_ded,tag=!redTeam,type=zombie] run scoreboard players set #decisionB value 2
...
execute if score #decision value matches 1 if score #decisionB value matches 1 at @e[limit=1,sort=random,tag=player_char,tag=!self_shmoves,tag=!player_ded,tag=!redTeam,type=zombie] run tag @e[limit=1,tag=squareNavOption,sort=nearest,type=area_effect_cloud] add nav_go_here

#2 -- not infected. try to evade infected players
scoreboard players set #decisionB value 1
...
execute if score #decision value matches 2 if score #decisionB value matches 1 at @e[limit=1,sort=nearest,tag=player_char,tag=!self_shmoves,tag=!player_ded,tag=redTeam,type=zombie] run tag @e[limit=1,tag=squareNavOption,sort=furthest,type=area_effect_cloud] add nav_go_here
