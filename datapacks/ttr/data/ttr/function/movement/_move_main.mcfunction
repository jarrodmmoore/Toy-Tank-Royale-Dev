...
#check "control/node" instead
#this was copied and split into 64 versions, one for each "player node"

#why? because using UUID is soo much faster than relying on @e for controlling our tank.
#the file bloat and copypasta is totally worth the performance gain. 100%.
...
#can't move if dead
scoreboard players set @s[scores={deathTime=1..}] left_right 0
scoreboard players set @s[scores={deathTime=1..}] up_down 0
...
execute as @e[limit=1,tag=my_char] at @s store result score @s char_x run data get entity @s Pos[0] 1
execute as @e[limit=1,tag=my_char] at @s store result score @s char_y run data get entity @s Pos[1] 1
execute as @e[limit=1,tag=my_char] at @s store result score @s char_z run data get entity @s Pos[2] 1
...
execute unless entity @s[scores={deathTime=1..}] unless entity @s[scores={haltMovement=1..}] run function ttr:movement/1_grounded
...
#tank aim
execute if entity @s[tag=validRotation,scores={deathTime=..0}] at @e[tag=my_char,limit=1] run function ttr:movement/aim/main
