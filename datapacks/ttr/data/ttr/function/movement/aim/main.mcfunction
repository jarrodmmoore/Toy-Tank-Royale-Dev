...
tp @e[limit=1,sort=nearest,tag=my_model_2,type=item_display,distance=..4] ~ ~0.5 ~ facing entity @e[limit=1,sort=nearest,type=marker,tag=lookyhere] eyes

scoreboard players set #recursions value 8
execute rotated as @e[limit=1,sort=nearest,tag=my_model_2,type=item_display,distance=..4] rotated ~ 0 positioned ~ ~.5 ~ run function ttr:movement/aim/show_aim
