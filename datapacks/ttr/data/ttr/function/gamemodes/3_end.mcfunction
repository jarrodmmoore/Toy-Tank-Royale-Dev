...
#execute as @a[tag=inControl] if score @s points >= #opt_scorelimit value run tellraw @a [{"translate":"ttr.player_wins","color":"white","bold":false,"with":[{"selector":"@s"}]}]
#execute as @e[tag=ai] if score @s aiPoints >= #opt_scorelimit value run tellraw @a [{"translate":"ttr.player_wins","color":"white","bold":false,"with":[{"selector":"@s"}]}]
#tellraw @a ["",{"text":" "}]
